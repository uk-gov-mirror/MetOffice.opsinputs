...
call f_conf % get_or_die("NumVarChannels", self % NumVarChannels)
allocate(self % varChannels(self % NumVarChannels))
  ...
  select case (VarFields(iVarField))
    ...
    case (VarField_britemp)
      call opsinputs_fill_fillreal2d( &
        ...
        self % compressVarChannels, self % size_of_varobs_array, self % varChannels)
    case (VarField_tskin)
    ...
    case (VarField_modelorog)
      ...
      call opsinputs_fill_fillrealfromgeoval( &
        Ob % Header % ModelOrog, "ModelOrog", Ob % Header % NumObsLocal, Ob % ModelOrog, &
        self % GeoVals, "height_above_mean_sea_level_at_surface", JediToOpsLayoutMapping)
    ...
    case (VarField_BriTempVarError)
      call opsinputs_fill_fillreal2d( &
        ...
        "ObsErrorData", self % compressVarChannels, self % size_of_varobs_array, self % varChannels)
